...
app.use(cors({
  origin: ['http://http://192.168.133.22:2002', 'https://api.aeonai.ai'],
...
let mongodbConnection = `mongodb://${dbConfig.HOST}:${dbConfig.PORT}/${dbConfig.DB}`;
if (process.env.DEV_MODE !== "true") {
  mongodbConnection = `mongodb://${dbConfig.USER}:${dbConfig.PASSWORD}@${dbConfig.HOST}:${dbConfig.PORT}/${dbConfig.DB}?directConnection=true&serverSelectionTimeoutMS=2000&authSource=admin`;
}
db.mongoose
...
app.use("/", (req, res) => {
  res.send("Welcome AeonZodiac api.");
});
